...
define([
    ...
    function onGameParametersUpdated() {
        ...
        var width = Number(style._style._width) * Number(style._style._transform._scale._x) / 1.5;
        var space = 8;
        var left;

        if (SKBeInstant.getGameOrientation() === "landscape") {
            left = (gr.lib._ticketCost._currentStyle._width - (length * width + (length - 1) * space) - 10) / 1.9;
        } else {
            left = (gr.lib._ticketCost._currentStyle._width - (length * width + (length - 1) * space) - 10) / 2;
    ...
    function onDisableUI() {
        plusButton.enable(false);
        minusButton.enable(false);
        if (SKBeInstant.config.jLotteryPhase !== 1 && prizePointList.length > 1 && !isPFS) {
            gr.lib._TicketCostDim.show(true);
        }
    }
